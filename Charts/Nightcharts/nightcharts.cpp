...
int Nightcharts::drawLegend(QPainter *painter)
{
    ...
    case Nightcharts::Vertical:
    {
        ...
        for (int i=pieces.size()-1;i>=0;i--)
        {
            ...
            painter->drawText(x+painter->fontMetrics().height()+dist,y+painter->fontMetrics().height()/2+dist,pieces[i].pname + " - " + QString::number((pieces[i].pPerc/sum)*100,'f',0)+"%");
        }
        break;
    }
